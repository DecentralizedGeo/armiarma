...
var (
	// Default Polygon-specific settings
	DefaultPolygonGossipTopics []string = []string{}
)

type PolygonCrawlerConfig struct {
...
// NewPolygonCrawlerConfig returns a new PolygonCrawlerConfig with default values
func NewPolygonCrawlerConfig() *PolygonCrawlerConfig {
	return &PolygonCrawlerConfig{
		...
		Bootnodes:                 DefaultPolygonBootnodes,
		PersistConnEvents:         DefaultPersistConnEvents,
	}
}

func (c *PolygonCrawlerConfig) Apply(ctx *cli.Context) {
	...
	}).Info("config for the Polygon crawler")
}

func (c *PolygonCrawlerConfig) Network() utils.NetworkType {
