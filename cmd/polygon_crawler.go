...
// PolygonCrawlerCommand contains the polygon sub-command configuration
var PolygonCrawlerCommand = &cli.Command{
	Name:   "polygon",
	Usage:  "crawl the Polygon (Bor) network",
	Action: LaunchPolygonCrawler,
	Flags: []cli.Flag{
		&cli.StringFlag{
			Name:        "log-level",
...
// LaunchPolygonCrawler is the function that is called when running `polygon`
func LaunchPolygonCrawler(c *cli.Context) error {
	log.Infoln("Starting Polygon Crawler...")

	conf := config.NewPolygonCrawlerConfig()
	conf.Apply(c)

	// Generate the Polygon crawler struct
	polygonCrawler, err := crawler.NewPolygonCrawler(c, *conf)
	if err != nil {
		return err
	}
