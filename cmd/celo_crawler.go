...
// CeloCrawlerCommand contains the celo sub-command configuration
var CeloCrawlerCommand = &cli.Command{
	Name:   "celo",
	Usage:  "crawl the Celo L2 network",
	Action: LaunchCeloCrawler,
	Flags: []cli.Flag{
		&cli.StringFlag{
			Name:        "log-level",
...
// LaunchCeloCrawler is the function that is called when running `celo`
func LaunchCeloCrawler(c *cli.Context) error {
	log.Infoln("Starting Celo Crawler...")

	conf := config.NewCeloCrawlerConfig()
	conf.Apply(c)

	// Generate the Celo crawler struct
	celoCrawler, err := crawler.NewCeloCrawler(c, *conf)
	if err != nil {
		return err
	}
